/**
 * @file Intel.h
 * @brief Intel VMX/EPT architectural layouts and per-processor state.
 * @warning Assembly-visible offsets are guarded by compile-time assertions.
 */
#pragma once
...
typedef struct _INTEL_HOST_STACK_FRAME {
    ...
    volatile ULONG64* ExitEntryTsc;
    volatile LONG* TscTrapArmed;
    ULONG64* LastGuestRdtsc;
    ULONG64* TscTrapCpuidEntryTsc;
    ULONG64 BareMetalCpuidCost;
...
typedef struct _INTEL_CPU_CONTEXT {
    ...
    volatile LONG TscTrapArmed;
    ULONG64 TscTrapCpuidEntryTsc;
    ULONG64 TscTrapCpuidExitPenalty;
    /* Trap-both-RDTSC stealth. The leaf-0 fast path sets TscTrapArmed and the
       RDTSC fast path consumes it. The first RDTSC in a probe pair records
       LastGuestRdtsc; the second (with the flag set) returns
       LastGuestRdtsc + BareMetalCpuidCost so the guest-visible delta
       equals just the bare-metal CPUID cost, independent of VM-exit latency
       or C-state wakeup.  TSC_OFFSET stays zero (never rewritten) to avoid
       monotonicity/drift. */
    ULONG64 LastGuestRdtsc;
...
C_ASSERT(sizeof(INTEL_GUEST_REGISTERS) == 15 * sizeof(ULONG64));
C_ASSERT(FIELD_OFFSET(INTEL_GUEST_REGISTERS, Rax) == 0);
C_ASSERT(FIELD_OFFSET(INTEL_GUEST_REGISTERS, Rdi) == 6 * sizeof(ULONG64));
C_ASSERT(FIELD_OFFSET(INTEL_GUEST_REGISTERS, R8) == 7 * sizeof(ULONG64));
C_ASSERT(FIELD_OFFSET(INTEL_GUEST_REGISTERS, R15) == 14 * sizeof(ULONG64));
C_ASSERT(sizeof(INTEL_HOST_STACK_FRAME) == 96);
...
C_ASSERT(FIELD_OFFSET(INTEL_HOST_STACK_FRAME, ExitEntryTsc) == 56);
C_ASSERT(FIELD_OFFSET(INTEL_HOST_STACK_FRAME, TscTrapArmed) == 64);
C_ASSERT(FIELD_OFFSET(INTEL_HOST_STACK_FRAME, LastGuestRdtsc) == 72);
C_ASSERT(FIELD_OFFSET(INTEL_HOST_STACK_FRAME, TscTrapCpuidEntryTsc) == 80);
C_ASSERT(FIELD_OFFSET(INTEL_HOST_STACK_FRAME, BareMetalCpuidCost) == 88);
...
C_ASSERT(FIELD_OFFSET(INTEL_CPU_CONTEXT, PrimaryControls) == 12660);
C_ASSERT(FIELD_OFFSET(INTEL_CPU_CONTEXT, TscTrapArmed) == 12896);
C_ASSERT(FIELD_OFFSET(INTEL_CPU_CONTEXT, TscTrapCpuidEntryTsc) == 12904);
C_ASSERT((FIELD_OFFSET(INTEL_CPU_CONTEXT, TscTrapArmed) & 3) == 0);
C_ASSERT((FIELD_OFFSET(INTEL_CPU_CONTEXT, TscTrapCpuidEntryTsc) & 7) == 0);
C_ASSERT((FIELD_OFFSET(INTEL_CPU_CONTEXT, TscTrapCpuidExitPenalty) & 7) == 0);
C_ASSERT(FIELD_OFFSET(INTEL_CPU_CONTEXT, LastGuestRdtsc) == 12920);
C_ASSERT((FIELD_OFFSET(INTEL_CPU_CONTEXT, LastGuestRdtsc) & 7) == 0);
