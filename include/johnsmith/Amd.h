/**
 * @file Amd.h
 * @brief AMD SVM architectural layouts and assembly entry contracts.
 * @warning Assembly-visible offsets are protected by the assertions below.
 */
#pragma once
...
/** @brief Packed VMCB control area defined by the AMD64 architecture manual. */
...
/** @brief Packed VMCB guest-state save area. */
...
/** @brief Per-processor SVM state shared by C and assembly entry paths. */
...
C_ASSERT(sizeof(AMD_GUEST_REGISTERS) == 15 * sizeof(ULONG64));
C_ASSERT(FIELD_OFFSET(AMD_GUEST_REGISTERS, Rax) == 0);
C_ASSERT(FIELD_OFFSET(AMD_GUEST_REGISTERS, Rdi) == 6 * sizeof(ULONG64));
C_ASSERT(FIELD_OFFSET(AMD_GUEST_REGISTERS, R8) == 7 * sizeof(ULONG64));
C_ASSERT(FIELD_OFFSET(AMD_GUEST_REGISTERS, R15) == 14 * sizeof(ULONG64));
...
/**
 * @brief Handles one SVM exit and selects resume or stop behavior.
 * @param[in,out] Registers Guest general-purpose register state.
 * @param[in,out] Cpu Common per-processor state for the exiting vCPU.
 * @return An `AMD_VMEXIT_ACTION` value consumed by the assembly path.
 */
ULONG
AmdVmExitHandler(
...
/**
 * @brief Enters SVM guest execution from the prepared host context.
 * @return Backend-defined launch status when control returns to C.
 */
ULONG
AmdAsmLaunch(
