...
DISP_XMM4           EQU -0A0h
DISP_XMM5           EQU -0B0h
DISP_FRAME_SIZE     EQU 0D0h    ; 0B0h locals + 20h outgoing home space
...
AsmHookDispatcher PROC FRAME
    ...
    mov     r10, [rbp+10h]
    test    rax, rax
    jnz     DispatchTargetReady
    lea     rax, DispatchReturn

DispatchTargetReady:
    ; R10 is restored, so its stacked slot can hold the transfer target.
    mov     [rbp+10h], rax
    ...
    ; Restore flags last. The stack-based transfer preserves every register
    ; and leaves the original return address ready for the selected target.
