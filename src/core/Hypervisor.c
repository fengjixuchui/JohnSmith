...
#include "johnsmith/Introspection.h"
...
static NTSTATUS
HvStartBackendServices(
    _Inout_ HV_STATE* State
    )
{
    if (State->Backend->StartServices == NULL) {
        return STATUS_SUCCESS;
    }
    return State->Backend->StartServices(State);
}

static VOID
HvStopBackendServices(
    _Inout_ HV_STATE* State
    )
{
    if (State->Backend->StopServices != NULL) {
        State->Backend->StopServices(State);
    }
}
    ...
    )
{
    ...
    *State = NULL;
    ...
    status = HvStartBackendServices(&HvGlobalState);
    if (!NT_SUCCESS(status)) {
    ...
    if (!NT_SUCCESS(status)) {
        HvStopBackendServices(&HvGlobalState);
