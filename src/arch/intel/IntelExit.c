    ...
    )
{
    ...
    if (InterlockedCompareExchange(&Context->TscTrapArmed, 0, 0) != 0 &&
        IntelTscStealthPairIsFresh(
            hostTsc, Context->TscTrapCpuidEntryTsc)) {
        if (backend == NULL) {
            KeBugCheckEx(
                HYPERVISOR_ERROR,
                INTEL_BUGCHECK_UNEXPECTED_EXIT,
        ...
        /* Trap-both-RDTSC model: return the first RDTSC of the probe pair
           plus the bare-metal CPUID cost so the guest-visible delta is
           independent of VM-exit latency.  Update LastGuestRdtsc for
           monotonicity so a later timestamp read cannot go backwards. */
        tsc = IntelTscStealthCompensatedRdtsc(
            Context->LastGuestRdtsc,
            backend->BareMetalCpuidCost);
        Context->LastGuestRdtsc = tsc;
    } else {
        tsc = hostTsc;
        Context->LastGuestRdtsc = tsc;
    }
    IntelDisarmTscTrap(Context);
    return tsc;
}
...
{
    ULONG reason = Context->LastExitReason;
    ...
    )
{
    ...
    registerRequest = (subleaf == IntelHypercallRegisterSubleaf());

    if (registerRequest) {
        ULONG64 sharedVirtualAddress = Registers->Rbx;
        (VOID)IntelHypercallWorkerEnqueueRegister(
            Context, PsGetCurrentProcess(), (PVOID)sharedVirtualAddress);
    ...
    for (commandIndex = 0;
         commandIndex < INTEL_HYPERCALL_CMD_COUNT;
         ++commandIndex) {
        if (page->CommandId ==
            IntelHypercallCommandId((INTEL_HYPERCALL_CMD)commandIndex)) {
            break;
        }
    }
    ...
    case (ULONG)INTEL_HYPERCALL_CMD_PROBE: {
        ...
        {
            NTSTATUS enqueueStatus = IntelHypercallWorkerEnqueue(
                (INTEL_HYPERCALL_CMD)commandIndex,
                Context,
                page,
                page->Args[0],
                page->Args[1]);
            if (enqueueStatus != STATUS_SUCCESS) {
                page->Result = (ULONG64)enqueueStatus;
                KeMemoryBarrier();
...
{
    const ULONG leaf = (ULONG)Registers->Rax;
    const ULONG subleaf = (ULONG)Registers->Rcx;
    int cpuid[4];

    /* Intel.asm routes leaf 1 through this handler in every configuration. */
    ...
    )
{
    ...
    if (reason == VMX_EXIT_RDTSC || reason == VMX_EXIT_RDTSCP) {
        ...
        tsc = IntelConsumeTscTrap(
            context, Cpu, reason, guestRip);
        if (reason == VMX_EXIT_RDTSCP) {
            Registers->Rcx = (ULONG)__readmsr(IA32_TSC_AUX);
        }
