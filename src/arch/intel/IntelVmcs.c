    ...
    )
{
     ...
     * existing case 1 handler in IntelExit.c.
     */
    __cpuidex(cpuid, 1, 0);
    ...
    requiredPrimary = VMX_PRIMARY_ACTIVATE_SECONDARY |
                      ...
                      VMX_PRIMARY_CPUID_EXITING |
                      VMX_PRIMARY_RDTSC_EXITING;
    desiredPrimary = requiredPrimary;
    /*
     * Trap-both TSC stealth: RDTSC/RDTSCP/RDMSR(IA32_TSC) exits are
     * required so the one-shot trap armed by every CPUID exit can return
     * (last guest RDTSC + calibrated bare-metal CPUID cost) on the next
     * timestamp read.  The guest-visible RDTSC;CPUID;RDTSC probe delta
     * then equals the bare instruction cost (~134 cycles on the
     * i5-12400F, well under HvD's 1000-cycle gate) regardless of
     * VM-exit latency or C-state wakeup.  RDTSCP exits when RDTSC
     * exiting is set together with the secondary Enable-RDTSCP control
     * (desiredSecondary).  CPUID interception remains required for
     * hypervisor-present-bit masking, the hypercall dispatch, and the
     * leaf-0 micropath that arms the trap.
     */
    ...
    hostRsp = ((ULONG64)context->HostStack + INTEL_HOST_STACK_SIZE -
               sizeof(INTEL_HOST_STACK_FRAME)) & ~0xfull;
    ...
    hostFrame->ExitEntryTsc = &context->LastExitEntryTsc;
    hostFrame->TscTrapArmed = &context->TscTrapArmed;
    hostFrame->LastGuestRdtsc = &context->LastGuestRdtsc;
    hostFrame->TscTrapCpuidEntryTsc = &context->TscTrapCpuidEntryTsc;
    hostFrame->BareMetalCpuidCost = backend->BareMetalCpuidCost;
    ...
    /* The leaf-0 micropath sets the one-shot CPUID-trap-pending flag
       before VMRESUME.  RDTSC exiting stays always-on for the trap-both
       model; the RDTSC fast path consumes the flag. */
    ...
    /* Intel SDM rev. 092, Vol. 3C sections 24.6.5 and 25.3. This field is
       fixed for the lifetime of the VMCS; runtime code never rewrites it. */
