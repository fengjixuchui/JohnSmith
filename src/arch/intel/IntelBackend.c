    ...
    )
{
    ...
    /* Trap-both model: RDMSR(IA32_TSC) is intercepted alongside
       RDTSC/RDTSCP so MSR-form timestamp reads consume the same one-shot
       trap and return (last guest RDTSC + bare-metal CPUID cost).  Writes
       stay native. */
    IntelSetMsrBitmapBit(Bitmap, INTEL_TSC_STEALTH_IA32_TSC, TRUE, FALSE);
...
static ULONG64
IntelCalibrateBareMetalCpuidCost(
    VOID
    )
{
    ULONG64 minimum = MAXULONGLONG;
    ULONG sample;
    int registers[4];

    /* This runs during backend preparation, before any processor executes
       VMXON. The minimum rejects interrupt and scheduling outliers. */
    for (sample = 0;
         sample < INTEL_TSC_STEALTH_CALIBRATION_SAMPLES;
         ++sample) {
        ULONG64 before;
        ULONG64 after;
...
static VOID
IntelCalibrateCpuidExitEntryPenalty(
    _Inout_ INTEL_CPU_CONTEXT* Context
    )
{
    const INTEL_BACKEND_CONTEXT* backend =
        (const INTEL_BACKEND_CONTEXT*)Context->BackendContext;
    ULONG64 minimum = MAXULONGLONG;
    ULONG sample;
    int registers[4];

    if (backend == NULL || backend->BareMetalCpuidCost == 0) {
        KeBugCheckEx(
            HYPERVISOR_ERROR,
    ...
    /* IntelAsmLaunch returns here in VMX non-root. Each CPUID arms the next
       RDTSC, so these samples expose the fixed CPUID VM-exit entry component
       that is absent from the pre-VMX bare-metal calibration. */
    Context->TscTrapCpuidExitPenalty = 0;
    for (sample = 0;
         sample < INTEL_TSC_STEALTH_CALIBRATION_SAMPLES;
         ++sample) {
        ULONG64 before;
    ...
    Context->TscTrapCpuidExitPenalty =
        IntelTscStealthDeriveExitEntryPenalty(
            minimum,
            backend->BareMetalCpuidCost);
    KeMemoryBarrier();
}
    ...
    )
{
    ...
    context->BareMetalCpuidCost = IntelCalibrateBareMetalCpuidCost();
    ...
    )
{
    ...
    *(ULONG*)context->Vmcs = (ULONG)(backend->VmxBasic & 0x7fffffffu);
    ...
    /* Trap-both invariant: the MSR bitmap MUST intercept RDMSR(IA32_TSC)
       so MSR-form timestamp reads consume the same one-shot trap as
       RDTSC/RDTSCP.  Dropping the interception would let a probe's second
       timestamp read expose the raw VM-exit span. */
    if (!IntelTscStealthMsrReadIsIntercepted(
            (const unsigned char*)context->MsrBitmap)) {
        IntelDestroyCpuContext(context);
    ...
    )
{
    ...
    return IntelHypercallWorkerStart();
}
    ...
    )
{
    ...
    if (IntelAsmLaunch() == 0) {
        IntelCalibrateCpuidExitEntryPenalty(context);
