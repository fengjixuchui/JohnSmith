...
C_ASSERT(HV_MAX_HOOKS * sizeof(HOOK_OBSERVE_QUERY_ENTRY) <=
    ...
    )
{
    ...
    if (Item->Command == INTEL_HYPERCALL_CMD_READ) {
    ...
    )
{
    ...
    if (Item->Command == INTEL_HYPERCALL_CMD_REGISTER) {
    ...
    if (Item->Command == INTEL_HYPERCALL_CMD_READ ||
        Item->Command == INTEL_HYPERCALL_CMD_WRITE) {
    ...
    case INTEL_HYPERCALL_CMD_INSTALL: {
        ULONG hookId = 0;
        NTSTATUS status = ObserveHookInstall(
            state,
            (PVOID)Item->Arg0,
            Item->Arg1,
            IntelHypercallHookCallback,
            &hookId);
        page->Result = NT_SUCCESS(status)
            ? (ULONG64)hookId
            : (ULONG64)status;
        break;
    }
    case INTEL_HYPERCALL_CMD_REMOVE: {
        NTSTATUS status = ObserveHookRemove(state, (ULONG)Item->Arg0);
        page->Result = (ULONG64)status;
        break;
    }
    case INTEL_HYPERCALL_CMD_QUERY_HOOK: {
        ...
        status = ObserveHookQuery((ULONG)Item->Arg0, query);
        page->Result = (ULONG64)status;
        break;
    }
    ...
    )
{
    ...
    /* The bounded wait keeps dispatcher work off the VM-exit stack. */
    ...
    )
{
    ...
    slot->Command = INTEL_HYPERCALL_CMD_REGISTER;
    ...
    )
{
    ...
    if ((Command != INTEL_HYPERCALL_CMD_INSTALL &&
         Command != INTEL_HYPERCALL_CMD_REMOVE &&
         Command != INTEL_HYPERCALL_CMD_READ &&
         Command != INTEL_HYPERCALL_CMD_WRITE &&
         Command != INTEL_HYPERCALL_CMD_QUERY_HOOK &&
         Command != INTEL_HYPERCALL_CMD_LIST_HOOKS &&
         Command != INTEL_HYPERCALL_CMD_PROBE) ||
