...
This document lists reachable first-party code included by the Visual Studio projects. Runtime behavior still requires validation on the target CPU and Windows build.
...
| Intel VMX | `src/intel.c`, `src/intel/intel_vmcs.c`, `src/intel/intel_exit.c`, `asm/intel.asm`. |
| Intel EPT/VPID | `src/intel/intel_slat.c` builds identity mappings, manages per-CPU views, performs generation-based invalidation. |
...
| Hook observation | `src/hook_observe.c`, `src/hook_thunk.c`, `src/hook_trampoline.c`, `asm/hook_dispatch.asm`. |
...
| AMD SVM/NPT | `src/amd.c`, `src/amd/`, `include/amd.h`, `asm/amd.asm`. |
...
Debug and Benchmark use the same solution with their respective `Configuration` value.
